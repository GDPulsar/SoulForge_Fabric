...
public class CylinderRenderer {
    ...
    public static void renderCylinder(Matrix4f model, VertexConsumer vertexConsumer, Vector3f a, Vector3f b, float radius, Color color, int overlay, int light, boolean dualFace) {
        ...
        for (int j = 0; j < CYLINDER_SEGMENTS; j++) {
            ...
            // Render the quad
            renderTri(model, vertexConsumer, p1, p2, p3, overlay, light, color);
            renderTri(model, vertexConsumer, p2, p3, p4, overlay, light, color);
            renderTri(model, vertexConsumer, p2, p1, a, overlay, light, color);
            renderTri(model, vertexConsumer, p4, p3, b, overlay, light, color);

            if (dualFace) {
                renderTri(model, vertexConsumer, p3, p2, p1, overlay, light, color);
                renderTri(model, vertexConsumer, p4, p3, p2, overlay, light, color);
                renderTri(model, vertexConsumer, a, p1, p2, overlay, light, color);
                renderTri(model, vertexConsumer, b, p3, p4, overlay, light, color);
            }
        }
    }
    ...
    private static void renderTri(
        ...
        consumer.vertex(model, p1.x, p1.y, p1.z).texture(0, 0).overlay(overlay).light(light).normal(0, 1, 0).color(color.getRGB());
        consumer.vertex(model, p2.x, p2.y, p2.z).texture(0, 0).overlay(overlay).light(light).normal(0, 1, 0).color(color.getRGB());
        consumer.vertex(model, p3.x, p3.y, p3.z).texture(0, 0).overlay(overlay).light(light).normal(0, 1, 0).color(color.getRGB());
    }
    ...
    public static void renderCylinder(VertexConsumer vertexConsumer, Vector3f a, Vector3f b, float radius, Color color, int overlay, int light) {
        ...
        for (int j = 0; j < CYLINDER_SEGMENTS; j++) {
            ...
            // Render the quad
            renderTri(vertexConsumer, p1, p2, p3, overlay, light, color);
            renderTri(vertexConsumer, p2, p3, p4, overlay, light, color);
            renderTri(vertexConsumer, p2, p1, a, overlay, light, color);
            renderTri(vertexConsumer, p4, p3, b, overlay, light, color);
        }
    }
    ...
    private static void renderTri(
        ...
        consumer.vertex(p1.x, p1.y, p1.z).texture(0, 0).overlay(overlay).light(light).normal(0, 1, 0).color(color.getRGB());
        consumer.vertex(p2.x, p2.y, p2.z).texture(0, 0).overlay(overlay).light(light).normal(0, 1, 0).color(color.getRGB());
        consumer.vertex(p3.x, p3.y, p3.z).texture(0, 0).overlay(overlay).light(light).normal(0, 1, 0).color(color.getRGB());
    }
}

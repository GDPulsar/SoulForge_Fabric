...
public record LeftClickPacket() implements CustomPayload {
    public static final CustomPayload.Id<LeftClickPacket> ID = new Id<>(SoulForgeNetworking.LEFT_CLICK);
    public static final PacketCodec<RegistryByteBuf, LeftClickPacket> CODEC = new PacketCodec<>() {
        @Override
        public LeftClickPacket decode(RegistryByteBuf buf) {
            return new LeftClickPacket();
        }

        @Override
        public void encode(RegistryByteBuf buf, LeftClickPacket value) {}
    };
    ...
    public static void receive(LeftClickPacket packet, ServerPlayNetworking.Context context) {
            ...
            if (heldItem.isOf(SoulForgeItems.BFRCMG)) {
                if (playerSoul.getMagic() >= 80f) {
                    ...
                    BlastEntity blast = new BlastEntity(player.getWorld(), Utils.getArmPosition(player), player, 0.5f,
                            Vec3d.ZERO, end, playerSoul.getLV()/10f * heldItem.get(SoulForgeItems.HEAT_COMPONENT),
                            Color.YELLOW, true, 4);
                    blast.owner = player;
                    ...
                    player.addVelocityInternal(player.getRotationVector().normalize().multiply(-1.5f));
                    player.velocityModified = true;
                    heldItem.set(SoulForgeItems.HEAT_COMPONENT, 0);
                }
            }

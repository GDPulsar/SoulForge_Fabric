...
abstract class PlayerEntityMixin extends LivingEntity {
    ...
    public void getEntityName(CallbackInfoReturnable<String> cir) {
        ...
        if (playerSoul.getDisguise() != null) {
            if (playerSoul.getDisguise() != player) {
                cir.setReturnValue(playerSoul.getDisguise().getNameForScoreboard());
            }
        }
    }
    ...
    protected float modifyOffGroundSpeed(float original) {
        PlayerEntity player = (PlayerEntity)(Object)this;
        if (player.getAttributes().hasAttribute(SoulForgeAttributes.AIR_SPEED)) {
            return (float)(original * player.getAttributeValue(SoulForgeAttributes.AIR_SPEED));
        }
        return original;
    }
}

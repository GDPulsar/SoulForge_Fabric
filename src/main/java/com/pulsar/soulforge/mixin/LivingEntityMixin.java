...
abstract class LivingEntityMixin extends Entity {
    ...
    private static void addAttributes(final CallbackInfoReturnable<DefaultAttributeContainer.Builder> info) {
        info.getReturnValue()
                .add(SoulForgeAttributes.AIR_SPEED);
    }

    @ModifyReturnValue(method = "getOffGroundSpeed", at=@At("RETURN"))
    protected float modifyOffGroundSpeed(float original) {
        LivingEntity living = (LivingEntity)(Object)this;
        if (living.getAttributes().hasAttribute(SoulForgeAttributes.AIR_SPEED)) {
            return (float)(original * living.getAttributeValue(SoulForgeAttributes.AIR_SPEED));
        }
        return original;
    }
}

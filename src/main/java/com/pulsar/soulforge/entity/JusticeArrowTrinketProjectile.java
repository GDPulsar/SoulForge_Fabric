...
public class JusticeArrowTrinketProjectile extends PersistentProjectileEntity {
    ...
    public JusticeArrowTrinketProjectile(World world, LivingEntity shooter) {
        super(SoulForgeEntities.JUSTICE_ARROW_TRINKET_TYPE, shooter, world, new ItemStack(SoulForgeItems.JUSTICE_ARROW), ItemStack.EMPTY);
    }
    ...
    public void tick() {
        ...
        if (this.age == 15) {
            for (int i = 0; i < 4; i++) {
                ArrowEntity arrowEntity = new ArrowEntity(this.getWorld(), this.getX(), this.getY(), this.getZ(), new ItemStack(Items.ARROW), ItemStack.EMPTY);

...
public class ImmobilizationEntity extends Entity implements Attackable {
    ...
    public ImmobilizationEntity(World world, Vec3d position, float health, LivingEntity target) {
        ...
        this.setSize((float)Math.max(target.getBoundingBox().getLengthX(), target.getBoundingBox().getLengthZ()), (float)target.getBoundingBox().getLengthY());
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        builder.add(SIZE, new Vector3f(1f, 2f, 1f));
    }

    @Override
    public void tick() {
        if (target != null) target.teleport(this.getX(), this.getY(), this.getZ(), false);
        super.tick();
    }

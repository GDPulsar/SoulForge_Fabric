...
public class BFRCMG extends MagicItem implements GeoItem {
        ...
        if (entity instanceof ServerPlayerEntity player) {
            if (selected) {
                if (stack.getOrDefault(SoulForgeItems.ACTIVE_COMPONENT, false)) {
                    ...
                    stack.set(SoulForgeItems.TIMER_COMPONENT, stack.getOrDefault(SoulForgeItems.TIMER_COMPONENT, 0) + 1);
                    if (stack.getOrDefault(SoulForgeItems.TIMER_COMPONENT, 0) % 4 == 0) {
                        ...
                        BlastEntity blast = new BlastEntity(player.getWorld(), Utils.getArmPosition(player), player, 0.1f,
                                Vec3d.ZERO, end, 2f + playerSoul.getLV() / 6f * (1 + 0.02f * stack.getOrDefault(SoulForgeItems.HEAT_COMPONENT, 0)),
                                Color.YELLOW, true, 4);
                        blast.owner = player;
                    ...
                    if (stack.getOrDefault(SoulForgeItems.TIMER_COMPONENT, 0) % 12 == 0) {
                        stack.set(SoulForgeItems.HEAT_COMPONENT, stack.getOrDefault(SoulForgeItems.HEAT_COMPONENT, 0) + 1);
                        stack.set(SoulForgeItems.TIMER_COMPONENT, 0);
                    }
                    if (stack.getOrDefault(SoulForgeItems.HEAT_COMPONENT, 0) == 20) {
            ...
            if (!stack.getOrDefault(SoulForgeItems.ACTIVE_COMPONENT, false) && stack.getOrDefault(SoulForgeItems.HEAT_COMPONENT, 0) > 0) {
                if (stack.getOrDefault(SoulForgeItems.TIMER_COMPONENT, 0) >= 40) {
                    stack.set(SoulForgeItems.HEAT_COMPONENT, stack.getOrDefault(SoulForgeItems.HEAT_COMPONENT, 0) - 1);
                    stack.set(SoulForgeItems.TIMER_COMPONENT, 0);
                } else {
                    stack.set(SoulForgeItems.TIMER_COMPONENT, stack.getOrDefault(SoulForgeItems.TIMER_COMPONENT, 0) + 1);
    ...
    public boolean isItemBarVisible(ItemStack stack) {
        return stack.getOrDefault(SoulForgeItems.HEAT_COMPONENT, 0) > 0;
    }

    public int getItemBarStep(ItemStack stack) {
        return Math.round(13.0F - (float)stack.getOrDefault(SoulForgeItems.HEAT_COMPONENT, 0) * 13.0F / 20f);
    }

    public int getItemBarColor(ItemStack stack) {
        float f = Math.min(20f, (float)stack.getOrDefault(SoulForgeItems.HEAT_COMPONENT, 0) / 20f);

...
public class FrostWave extends MagicItem {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (user.getServer() != null) {
            ServerWorld serverWorld = user.getServer().getWorld(world.getRegistryKey());
            if (serverWorld != null) {
                ...
                for (LivingEntity entity : Utils.getEntitiesInFrontOf(user, 3f, 7f, 1f, 2f)) {
                    ...
                    entity.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, 80 + playerSoul.getEffectiveLV()*2, Math.round(playerSoul.getEffectiveLV()/5f) - 1));
                    entity.addStatusEffect(new StatusEffectInstance(SoulForgeEffects.VULNERABILITY, 80 + playerSoul.getEffectiveLV()*2, Math.round(playerSoul.getEffectiveLV()/6f) - 1));
                    entity.damage(SoulForgeDamageTypes.of(user.getWorld(), SoulForgeDamageTypes.ABILITY_DAMAGE_TYPE), 1f);
                    int duration = 10;
                ...
                serverWorld.playSoundFromEntity(null, user, SoulForgeSounds.FROST_WAVE_EVENT, SoundCategory.PLAYERS, 1f, 1f);
                for (int i = 0; i < 5; i++) {
                    serverWorld.addParticle(ParticleTypes.SNOWFLAKE,

...
public class DeterminationHammer extends MagicSwordItem implements GeoItem {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        BlockHitResult hit = world.raycast(new RaycastContext(user.getEyePos(), user.getEyePos().add(user.getRotationVector().multiply(user.getEntityInteractionRange())), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, user));
        if (!world.isClient) {
            if (hit != null) {
                if (user.isSneaking()) {

...
public class JusticeHarpoon extends MagicSwordItem/* implements GeoItem*/ {
    public JusticeHarpoon() {
        super(5, 1.2f, 0.2f);
        addAttribute(EntityAttributes.PLAYER_ENTITY_INTERACTION_RANGE, new EntityAttributeModifier(Identifier.of( SoulForge.MOD_ID, "jt_harpoon_reach"), 2f, EntityAttributeModifier.Operation.ADD_VALUE));
    }
        ...
        for (JusticeHarpoonProjectile harpoon : world.getEntitiesByClass(JusticeHarpoonProjectile.class, Box.of(user.getPos(), 200, 200, 200), (entity) -> entity.getOwner() == user)) {
            if (harpoon.stuckEntity != null) {
                if (user.isSneaking()) {
                    ...
                    harpoon.stuckEntity.addVelocityInternal(offset.normalize().multiply(Math.sqrt(offset.length() * 1.5f)).add(0f, 1.25f, 0f));
                } else {
                    Vec3d offset = user.getPos().subtract(harpoon.stuckEntity.getPos());
                    harpoon.stuckEntity.addVelocityInternal(offset.normalize().multiply(Math.sqrt(offset.length() * 4f)));

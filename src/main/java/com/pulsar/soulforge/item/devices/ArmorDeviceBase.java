...
public class ArmorDeviceBase extends ArmorItem implements Equipment {
    ...
    public ArmorDeviceBase(RegistryEntry<ArmorMaterial> material, ArmorItem.Type type, Item.Settings settings, int maxCharge, TraitBase trait) {
    ...
    public int getCharge(ItemStack stack) {
        return ((stack.getItem() instanceof DeviceBase || stack.getItem() instanceof ArmorDeviceBase) && stack.contains(SoulForgeItems.CHARGE_COMPONENT)) ? stack.get(SoulForgeItems.CHARGE_COMPONENT) : 0;
    }

    public void setCharge(ItemStack stack, int charge) {
        if (stack.getItem() instanceof DeviceBase || stack.getItem() instanceof ArmorDeviceBase) {
            stack.set(SoulForgeItems.CHARGE_COMPONENT, Math.min(Math.max(charge, 0), maxCharge));
        }
    }
    ...
    public void tryCharge(PlayerEntity player) {
        ItemStack stack;
        if (player.getMainHandStack().getItem() instanceof DeviceBase || player.getMainHandStack().getItem() instanceof ArmorDeviceBase) stack = player.getMainHandStack();
        else if (player.getOffHandStack().getItem() instanceof DeviceBase || player.getMainHandStack().getItem() instanceof ArmorDeviceBase) stack = player.getOffHandStack();
        else return;
        if (getCharge(stack) < maxCharge) {

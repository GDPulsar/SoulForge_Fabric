...
public class HealTablet extends DeviceBase implements GeoItem {
    public HealTablet() {
        super(new FabricItemSettings().maxCount(1).rarity(Rarity.RARE), 300, Traits.kindness);
    }
                ...
                if (!user.isSneaking()) {
                    ...
                    if (hit != null) {
                        if (hit.getEntity() instanceof LivingEntity target) {
                            stack.getNbt().putUuid("target", target.getUuid());
                            stack.getNbt().putInt("timer", 200);
                    ...
                    stack.getNbt().putUuid("target", user.getUuid());
                    stack.getNbt().putInt("timer", 200);
                    user.getWorld().playSound(null, user.getX(), user.getY(), user.getZ(), SoulForgeSounds.HEAL_EVENT, SoundCategory.PLAYERS, 1f, 1f);
                    for (int i = 0; i < 20; i++) {
                        user.getWorld().addParticle(new DustParticleEffect(Vec3d.unpackRgb(0x00FF00).toVector3f(), 1f),
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (!world.isClient) {
            stack.getOrCreateNbt();
            assert stack.getNbt() != null;
            if (stack.getNbt().contains("timer") && stack.getNbt().contains("target")) {
                LivingEntity target = (LivingEntity) ((ServerWorld) world).getEntity(stack.getNbt().getUuid("target"));
                if (target != null) {
                    if (target.age - target.getLastAttackedTime() >= 0 && target.age - target.getLastAttackedTime() <= 1) {
                        stack.getNbt().remove("target");
                        stack.getNbt().remove("timer");
                        return;
                    }
                    int timer = stack.getNbt().getInt("timer");
                    if (timer <= 0) return;
                    stack.getNbt().putInt("timer", timer - 1);

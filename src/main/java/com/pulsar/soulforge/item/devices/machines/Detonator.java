...
public class Detonator extends DeviceBase implements GeoItem {
    public Detonator() {
        super(new Item.Settings().maxCount(1).rarity(Rarity.EPIC), 1000, Traits.bravery);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            ...
            if (!nbt.contains("mines")) {
                nbt.put("mines", new NbtList());
            }
            List<DetonatorMine> mines = new ArrayList<>();
            int[] array = nbt.getIntArray("mines");
            for (int hash : array) {

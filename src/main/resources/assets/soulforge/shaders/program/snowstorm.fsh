...
vec3 lerp(float delta, vec3 a, vec3 b) {
    return a + delta*(b-a);
}

vec3 fog(float depth, vec3 a, vec3 b) {
    return lerp(min(1.0, max(0.0, depth-2.0)), a, b);
}
...
void main() {
    ...
    float texelAccum = depth_layers[0];
    for ( int ii = 1; ii < active_layers; ++ii ) {
        texelAccum += depth_layers[ii];
    }
    texelAccum /= active_layers;

    float depth = texelAccum;
    depth = linearDepth(depth);
    vec3 normalColor = texture(DiffuseSampler, texCoord).rgb;
    vec3 snowColor = vec3(0.8, 0.8, 1.0);
    vec3 color = fog(depth, normalColor, snowColor);
